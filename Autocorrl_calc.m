...
function current_autocorr_array = Autocorrl_calc(curr_ID, curr_scenario)
%% Init
...
for indx = 1:length(IDrange)
    ...
    for sz = 1:length(scenarios)
        ...
        if isempty(found)
            fprintf('---- skipped\n');
            current_autocorr_array = 0; % Customized by Soumalya Bose to avoid Technical Glitches
            continue
        end
        ...
        current_autocorr_array = autocorr_values;
    end
end
end